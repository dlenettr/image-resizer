...
class SimpleImage {
    ...
    function sizes( ) {
        return array( 'w' => intval( $this->width ), 'h' => intval( $this->height ) );
    }

    function smart_crop( $w, $h, $align = "t-l" ) {
        ...
        if ( $w >= $this->width ) { $x2 = $this->width; }
        //else $x2 = $w;
        if ( $h >= $this->height ) { $y2 = $this->height; }
        //else $y2 = $h;
        ...
        imagealphablending( $new, false );
        imagesavealpha( $new, true );
        imagecopyresampled( $new, $this->image, 0, 0, $x1, $y1, $w, $h, $w, $h );
